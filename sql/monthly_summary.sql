...
monthly_stats AS (
  ...
  FROM main.account_monitoring_dev.dashboard_data d
...
-- Main summary
SELECT
  (SELECT month_start FROM prev_month) as report_month,
...
SELECT
  ...
  monthly_cost,
  prev_month_cost,
  ROUND(monthly_cost - prev_month_cost, 2) as cost_change,
