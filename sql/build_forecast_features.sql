...
CREATE OR REPLACE VIEW IDENTIFIER({{catalog}} || '.' || {{schema}} || '.forecast_features') AS
WITH daily_costs AS (
  SELECT
    c.contract_id,
    c.account_id,
...
CREATE OR REPLACE VIEW IDENTIFIER({{catalog}} || '.' || {{schema}} || '.forecast_feature_summary') AS
SELECT
  contract_id,
  cloud_provider,
  contract_start,
...
FROM IDENTIFIER({{catalog}} || '.' || {{schema}} || '.forecast_features')
GROUP BY
  contract_id,
  cloud_provider,
  contract_start,
...
FROM IDENTIFIER({{catalog}} || '.' || {{schema}} || '.forecast_feature_summary');
