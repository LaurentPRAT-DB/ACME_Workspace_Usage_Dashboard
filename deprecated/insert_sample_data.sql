...
WHEN NOT MATCHED THEN INSERT (
  account_id, customer_name, business_unit_l0, business_unit_l1,
  ...
  source.account_id, source.customer_name, source.business_unit_l0,
...
-- Insert single sample contract (1694992)
-- Set to start 1 year ago and end 1 year from now (2-year contract)
MERGE INTO main.account_monitoring_dev.contracts AS target
USING (
  SELECT
    '1694992' as contract_id,
    aa.account_id,
    ac.cloud_provider,
    DATE_SUB(CURRENT_DATE(), 365) as start_date,
    DATE_ADD(CURRENT_DATE(), 365) as end_date,
    3000.00 as total_value,
    'USD' as currency,
    'SPEND' as commitment_type,
    'ACTIVE' as status,
    '2-year $3000 sample contract for burn down demo' as notes,
    CURRENT_TIMESTAMP() as created_at,
    CURRENT_TIMESTAMP() as updated_at
  FROM actual_account aa
